            ...
            <div class="card-body">
                <ul class="nav nav-tabs" id="myTab2" role="tablist">
                    @foreach($languages as $language)
                        <li class="nav-item">
                            <a class="nav-link {{ $loop->index === 0 ? 'active' : '' }}"
                               id="home-tab2" data-toggle="tab" href="#dtable-{{ $language->lang }}" role="tab"
                               aria-controls="home" aria-selected="true">{{ $language->name }}</a>
                        </li>
                    @endforeach
                </ul>
                <div class="tab-content tab-bordered" id="myTab3Content">
                    @foreach($languages as $language)
                        @php
                            $footerGridOne = \App\Models\FooterGridThree::where('language', $language->lang)
                            ->orderByDesc('id')->get()
                        @endphp
                        <div class="tab-pane fade show {{ $loop->index === 0 ? 'active' : '' }}"
                                    ...
                                    <table class="table table-striped" id="table-{{ $language->lang }}">
                                        ...
                                        @foreach($footerGridOne as $footer)
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="{{ route('admin.footer-grid-three.edit', $footer->id) }}" class="btn btn-primary">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <a href="{{ route('admin.footer-grid-three.destroy', $footer->id) }}"
